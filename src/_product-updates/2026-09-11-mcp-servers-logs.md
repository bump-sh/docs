...
You can also surface custom execution data by defining custom analytics in your workflow definition document. Runtime expressions let you pick the inputs, outputs or configuration values, and inspect them without opening a debug session. Authentication data is always excluded. Logs are stored anonymously, with nothing tying them back to personal data.
