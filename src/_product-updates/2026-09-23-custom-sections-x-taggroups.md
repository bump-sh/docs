...
Until now, operations and groups of operations sat under the same “Endpoints”, “Webhooks” and “Channels” sections, That's fine on a small API, but for APIs covering multiple business use cases, it can get noisy really quick.

The `x-tagGroups` vendor extension let you add a level to your navigation by defining your own sections.
...
`x-tagGroups` works for both OpenAPI and AsyncAPI documents, regardless of their version. If you're already using OpenAPI 3.2, you can [customize your sections natively using the `kind: nav` tags](/openapi/v3.2/documentation/grouping-operations-with-tags/#nested-tag-structures).
