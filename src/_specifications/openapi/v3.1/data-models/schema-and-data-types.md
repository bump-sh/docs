...
In addition to defining data types and formats, [JSON Schema](_specifications/openapi/v3.1/data-models/json-schema.md) provides several validation keywords to enforce specific constraints on the data. Here are a few popular validation keywords:
...
> Learn more about `const` on [JSON-Schema.org: Constant Values](https://json-schema.org/understanding-json-schema/reference/const), and read our guide on [Schema Composition](_specifications/openapi/v3.1/data-models/schema-composition.md) to learn more about `oneOf`.
{: .info }
...
There is a lot more to JSON Schema and OpenAPI Schema Objects than we've covered here, but this will hopefully get you off to a good start. If you need to learn more, you can read our guide on [JSON Schema in OpenAPI](_specifications/openapi/v3.1/data-models/json-schema.md).
