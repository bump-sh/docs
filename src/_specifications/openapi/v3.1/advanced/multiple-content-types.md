...
authors: phil
excerpt: 
canonical_url: https://docs.bump.sh/openapi/v3.2/advanced/multiple-content-types/
date: 2024-08-07
---
...
In [Understanding Structure > Paths & Operations](_specifications/openapi/v3.1/understanding-structure/paths-operations.md) we learned how to create a simple API with both `application/json` and `text/csv`, so lets take that example and expand it a little.
...
The API documentation tool has named the XML tag `<Bookings>` from the component name, but you can change that if [learn about the `xml` keyword](_specifications/openapi/v3.1/data-models/representing-xml.md), which can also handle more advanced XML syntax.
