...
Paths can store variables, a little bit like the concept of [server variables](_specifications/openapi/v3.1/understanding-structure/api-servers.md), using curly braces `{}` as a placeholder for a parameter which will be defined within the operation.
...
The responses are then broken down by status code, and again all the responses can have [multiple content types](_specifications/openapi/v3.1/advanced/multiple-content-types.md). Then the content can be further described by a `schema`, and an `example` (or `examples`).
...
- Learn more about defining [HTTP requests](_specifications/openapi/v3.1/understanding-structure/http-requests.md) and [HTTP responses](_specifications/openapi/v3.1/understanding-structure/http-responses.md).
- Learn more about [schemas and data types](_specifications/openapi/v3.1/data-models/schema-and-data-types.md).
