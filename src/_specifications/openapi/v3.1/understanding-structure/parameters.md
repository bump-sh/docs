...
canonical_url: https://docs.bump.sh/openapi/v3.2/understanding-structure/parameters/
date: 2024-07-04
---
...
> In previous versions of OpenAPI the entire request body and form data would all be sent as parameters, but since OpenAPI v3.0 this has been moved to the content object. Learn more in [HTTP Requests](_specifications/openapi/v3.1/understanding-structure/http-requests.md).
{: .info }
...
Here is one required path parameter, `bookingId`, with its `name` matching `{bookingId}`. The `schema` can contain anything you'd [expect to find in schema](_specifications/openapi/v3.1/data-models/schema-and-data-types.md), from data types to other validations.
...
Cookie parameters can be any primitive values, arrays and objects. Arrays and objects are serialized using the form style. For more information, see [Parameter Serialization](_specifications/openapi/v3.1/understanding-structure/parameter-serialization.md).

The first thought might be to use cookie for authentication, but for that you would be better off using [API keys](_specifications/openapi/v3.1/advanced/security.md). Cookie parameters are reserved for other things, like tracking and analytics, locale preferences, or other session related information which does not fit into the HTTP specification with dedicated headers.
