...
This example references the `HttpBearerToken` security scheme defined in the `components` section. This is useful for keeping an OpenAPI document DRY (Don't Repeat Yourself), especially when the components are [spread across multiple documents](_specifications/openapi/v3.2/advanced/splitting-documents-with-ref.md).
