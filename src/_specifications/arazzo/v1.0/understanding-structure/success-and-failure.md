...
**context (required when type == regex or jsonpath)** - Which bit of the data are we evaluating. This could be `$response.body`, `$response.headers`, or any other valid [runtime expressions](_specifications/arazzo/v1.0/working-with-arazzo/runtime-expressions.md).
...
Actions can be defined inline or referenced from [reusable components](_specifications/arazzo/v1.0/understanding-structure/components-and-references.md) to maintain consistency across workflows.
